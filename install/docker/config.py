...
HDFS_CONFIG = {
    ...
    'keytab_path': os.getenv('HDFS_KEYTAB_PATH', '/etc/security/keytabs/hdfs.keytab'),
    'principal': os.getenv('HDFS_PRINCIPAL', 'hdfs@REALM.COM'),
    'base_path': os.getenv('HDFS_BASE_PATH', '/user/hive/warehouse'),
    'timeout': int(os.getenv('HDFS_TIMEOUT', '30')),
}

# 数据集本地存储路径 (HDFS 下载后存放)
DATASET_SAVEPATH = os.getenv('DATASET_SAVEPATH',
    os.path.join(WORKSPACE_HOST_PATH, 'datasets') if 'WORKSPACE_HOST_PATH' in dir() else '/data/k8s/kubeflow/datasets')

# 数据集预览行数
DATASET_PREVIEW_ROWS = int(os.getenv('DATASET_PREVIEW_ROWS', '10'))
