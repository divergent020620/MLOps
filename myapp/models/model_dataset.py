...
class Dataset(Model,AuditMixinNullable,MyappModelBase):
    ...
    hdfs_path = Column(String(1000), nullable=True, comment='HDFS源路径(相对base_path)')
...
'''
    ...
    def preview_html(self):
        """数据预览 - 点击跳转到前端概览页面"""
        return Markup(
            f'<a href="/frontend/datasetOverview?id={self.id}" '
            f'target="_blank" '
            f'style="color:#1890ff;border:1px solid #1890ff;background:#fff;'
            f'padding:2px 8px;border-radius:4px;font-size:12px;text-decoration:none">'
            f'预览</a>'
        )
