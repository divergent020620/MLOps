...
class MyappModelRestApi(ModelRestApi):
    ...
    def download(self):
        ...
        # 下载全量 (分块读取避免大表 OOM，限制最大 200000 行)
        DOWNLOAD_MAX_ROWS = 200000
        sql = f'select * from {table_name} limit {DOWNLOAD_MAX_ROWS}'
        print(sql)
        # 分块读取 + 追加写入
        first_chunk = True
        for chunk in pandas.read_sql_query(sql, sql_engine, chunksize=10000):
            chunk.to_csv(
                csv_file_path,
                index=False,
                sep=",",
