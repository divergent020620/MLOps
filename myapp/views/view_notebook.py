...
class Notebook_ModelView_Base():
    ...
    def entry_jupyter(self):
        ...
        if not exist_pod:
            ...
            pod, pod_spec = k8s_client.make_pod(
                ...
                resource_memory="2G~" + notebook.resource_memory,
                resource_cpu="1~" + notebook.resource_cpu,
    ...
    def reset_theia(self, notebook):
        ...
        k8s_client.create_debug_pod(
            ...
            node_selector=notebook.get_node_selector(),
            resource_memory=notebook.resource_memory if conf.get('NOTEBOOK_EXCLUSIVE',False) else ("2G~" + notebook.resource_memory),
            resource_cpu=notebook.resource_cpu if conf.get('NOTEBOOK_EXCLUSIVE',False) else ("1~" + notebook.resource_cpu),
            resource_gpu=notebook.resource_gpu,
            image_pull_policy=conf.get('IMAGE_PULL_POLICY', 'Always'),
