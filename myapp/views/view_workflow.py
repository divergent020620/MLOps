...
class Workflow_ModelView_Base():
    ...
    def stop(self, crd_id):
        ...
        # 只有管理员和创建者可以
        if workflow.username==g.user.username or g.user.is_admin() or (workflow.pipeline and workflow.pipeline.project.user_role(g.user.id)== 'creator'):
    ...
    def web_node_detail(self,cluster_name,namespace,workflow_name,node_name):
        ...
        tip = __("提示：支持任务结果、模型指标、数据集可视化预览")
        tab7 = [
            {
                "tabName": __("结果可视化"),
                "content": [
