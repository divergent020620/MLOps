...
import type { AnnouncementData } from './components/Announcement/AnnouncementModal';
...
const AppWrapper = (props: IProps) => {
  ...
  // ---- 公告 Badge 相关 state ----
  // 当前生效公告数据（由 AnnouncementModal 的 onDismiss 回调传入）
  const [badgeAnnouncement, setBadgeAnnouncement] = useState<AnnouncementData | null>(null);

  // ---- 公告弹窗相关 state ----
  // announcementModalKey 变化时强制重新挂载 AnnouncementModal（Badge 点击触发）
  const [announcementModalKey, setAnnouncementModalKey] = useState(0);
  // forceOpen=true 时跳过 localStorage 检查，有公告就一定弹窗（Badge 点击时置 true）
  ...
  // ---- 公告弹窗关闭回调：更新 Badge 状态 ----
  const handleAnnouncementDismiss = useCallback((ann: AnnouncementData | null) => {
    setForceOpenAnnouncement(false);  // 重置 forceOpen
    setBadgeAnnouncement(ann);        // 更新铃铛状态
  }, []);

  // ---- 公告 Badge 点击：强制重新打开弹窗 ----
  const handleBadgeClick = () => {
    setForceOpenAnnouncement(true);
    setAnnouncementModalKey((prev) => prev + 1);
  };
  ...
  return (
    <div className="content-container fade-in">
      ...
      <div className="main-content-container">
        ...
        {/* 公告弹窗 —— 全局层级，所有页面都生效 */}
        <AnnouncementModal
          key={announcementModalKey}
          forceOpen={forceOpenAnnouncement}
          onDismiss={handleAnnouncementDismiss}
        />

        {/* 公告铃铛浮动按钮 —— 固定在页面右下角，在自定义按钮上方 */}
        <AnnouncementBadge
